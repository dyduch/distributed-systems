...
int main(int argc, char** argv){
    ...
    if(protocol_type == UDP){
        ...
        sendto(udp_socket, &init_token, sizeof(init_token), 0, (struct sockaddr *) &next_addr, sizeof(next_addr));
        ...
        while(running){
            ...
            if(recv_token.type == INIT_TOKEN){
                printf("\n[INIT_TOKEN] from: %s:%d!\n"
                       ...
                       ,recv_token.sender_id, recv_token.from, recv_token.to);
                if(recv_token.from == my_port){
                    ...
                    if(recv_token.my_next_client_port == next_client_port){
                        ...
                        sendto(udp_socket, &recv_token, sizeof(recv_token), 0, (struct sockaddr *) &next_addr, sizeof(next_addr));
                ...
                if(recv_token.to != my_port){
                    ...
                    printf("\n[MSG_TOKEN] from %s:%d!\n"
                           "[MSG_TOKEN] text: %s\n"
                           "[MSG_TOKEN] SENDING new message to: %d\n\n", recv_token.sender_id, recv_token.from, recv_token.msg, next_client_port);

                    char msg[] = "Random message";
                    recv_token = get_msg_token(client_id, my_port, next_client_port, msg);
                    sendto(udp_socket, &recv_token, sizeof(recv_token), 0, (struct sockaddr *) &next_addr, sizeof(next_addr));
                }
            }
...
void init_client(int argc, char** argv){
    ...
    if(my_port < (uint16_t) MIN_PORT_NUMBER|| my_port > (uint16_t) (MIN_PORT_NUMBER + MAX_CLIENTS_AMOUNT - 1)){
    ...
    if(next_client_port < (uint16_t) MIN_PORT_NUMBER|| next_client_port > (uint16_t) (MIN_PORT_NUMBER + MAX_CLIENTS_AMOUNT - 1)){
        printf("Wrong number of port!\n");
        exit(1);
    }
...
//    if(network_size == 1 && has_token_initially == 0){
//        printf("First client should have token initially\n");
//        release_port(my_port);
//        exit(1);
//    }

    if(network_size != 1 && has_token_initially == 1){
        printf("TODO: init new client with has token initially set to 1\n");
        release_port(my_port);
        exit(1);
    }
//
//    if(taken_ports[next_client_port - MIN_PORT_NUMBER] == 0){
//        printf("There is no such client in network on port: %d\n", next_client_port);
//        release_port(my_port);
//        exit(1);
//    }
    ...
    printf("Client %s initialized succesfully with parameters:\n"
           ...
           "Next Port: %d\n"
           "Has token: %d\n"
           "Protocol: %d\n\n", client_id, my_port, next_client_ip, next_client_port, has_token_initially, protocol_type);

    //calculate_receiver();
}

// no idea why it works but works
